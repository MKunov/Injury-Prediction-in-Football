...
class MTO_SW_LSTM(nn.Module):
    ...
    def forward(self, x, using_gpu, nseq):
        ...
        newlengths = []
        print("Input shape to LSTM:", x_padded.shape)
    ...
    def train_model(self, train_data, val_data, epochs, bsize, learning_rate, using_gpu, testnbr):
        ...
        for ee in range(epochs):
            ...
            if (ee+1)%testnbr==0 or ee==0:
                ...
                precision, recall, _ = precision_recall_curve(y_val, out)
                pr_auc = auc(recall, precision)   
                print("Epoch {} - Training loss: {} - Validation loss: {}".format(ee+1, mean_train_loss, val_loss_list[-1]))
                print("F1 score: {} - Confusion Matrix: {} - Precision: {} - Recall: {} - pr_auc: {}".format(f1, conf_mat, precision, recall, pr_auc))
        print("Finished training in ", time.time()-time0, " seconds")
        print("F1 score: {} - Confusion Matrix: {} - Precision: {} - Recall: {} - pr_auc: {}".format(f1, conf_mat, precision, recall, pr_auc))
        print("F1 score:", f1)
        print("Confusion Matrrix: ", conf_mat)
        print(classification_report(y_val, y_pred))

        return f1, conf_mat, precision, recall, pr_auc 
    
    
"""

    def evaluate_metrics(self, data, threshold=0.5):
        self.eval()
        x_val = [obj[0] for obj in data]
        y_true = torch.cat([obj[1] for obj in data], dim=0).cpu().numpy()
        nvalseq = len(x_val)
        with torch.no_grad():
            y_pred_prob = self.forward(x_val, self.device == 'cuda', nvalseq).cpu().numpy()
