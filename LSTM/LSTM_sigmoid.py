...
def create_sequences_with_ids(data, window_size):
    sequences = []
    targets = []
    player_ids = []
    ...
    return np.array(sequences), np.array(targets), np.array(player_ids)



sequences, targets, player_ids = create_sequences_with_ids(data, window_size)
...
X_train, X_test, y_train, y_test, player_ids_train, player_ids_test = train_test_split(sequences, targets, player_ids, test_size=0.2, random_state=42)
...
sigmoid_output = model_lstm.predict(X_test_scaled)

print(type(y_test), y_test.shape)
print(type(sigmoid_output), sigmoid_output.shape)

# Assuming you have sigmoid_output and player_ids_test corresponding to X_test_scaled
for unique_id in np.unique(player_ids_test):
    print(f"Player {unique_id}:")
    player_predictions = sigmoid_output[player_ids_test == unique_id]
    for i, prediction in enumerate(player_predictions):
        print(f"\tSequence {i + 1}: {prediction[0]}")
    print("\n")


import tensorflow as tf

# Assuming `model_lstm` is your trained model and `X_test_scaled` is your input data
# Choose a specific example to investigate
example = X_test_scaled[0:1]  # Select a single sequence for analysis

# Watch the input tensor
with tf.GradientTape() as tape:
    tape.watch(example)
    prediction = model_lstm(example)

# Calculate gradients of the prediction with respect to the input
gradients = tape.gradient(prediction, example)

# The gradients can give you an idea of the feature importance
# Higher gradients mean the prediction is more sensitive to the feature
print(gradients)


# Evaluation metrics
print("ROC AUC score", roc_auc_score(y_test, sigmoid_output))
